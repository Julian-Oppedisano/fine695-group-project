...
# --- Configuration ---
MODEL_NAME = "CatBoost_AE_eps_surprise" # Changed model name
...
# --- CSV Logging Function (same as before, ensures RMSE logging if present) ---
RESULTS_DIR = 'results'
...
def log_metrics_to_csv(model_name, metrics_dict):
    ...
    # Ensure RMSE keys are present if any RMSE data exists, and that they are correctly ordered
    ...
    if has_rmse:
        ...
        metric_keys = sorted(metric_keys) # Re-sort after potential append
    ...
    with open(CSV_FILE, 'a', newline='') as csvfile:
        ...
        for key in fieldnames: # Iterate through fieldnames to ensure order and presence
            if key not in ('timestamp', 'model_name'):
                 row_to_write[key] = metrics_dict.get(key) # Use .get() for safety
...
def main():
    ...
    # AE factors are already scaled/imputed by the generation script. No further scaling for CatBoost.

    print("Training CatBoostRegressor model on AE factors...")
    ...
    print("\n--- CatBoost Model Feature Importance for AE Factors ---")
    if hasattr(cb_model, 'get_feature_importance'):
        ae_predictor_cols = [f'ae_factor_{i}' for i in range(X_train.shape[1])]
        feature_importances = pd.Series(cb_model.get_feature_importance(), index=ae_predictor_cols)
        sorted_importances = feature_importances.sort_values(ascending=False)
        print("Top AE factors by importance (max 32):")
